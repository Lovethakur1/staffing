...
interface AppStateContextType {
  ...
  setTotalBreakMinutes: (mins: number) => void;
...
export function AppStateProvider({
  ...
  // Restore active shift state on page refresh
  useEffect(() => {
    ...
    api.get('/shifts').then((res) => {
      ...
      // Find shift that is either IN_PROGRESS or on BREAK
      const activeShiftData = shifts.find((s: any) => 
        (s.status === 'IN_PROGRESS' || s.status === 'BREAK') && s.clockIn
      );
      if (!activeShiftData) return;

      const mapped: Shift = {
        id: activeShiftData.id,
        eventId: activeShiftData.eventId,
        staffId: activeShiftData.staffId,
        date: activeShiftData.date,
        startTime: activeShiftData.startTime,
        endTime: activeShiftData.endTime,
        status: 'ongoing' as const,
        location: activeShiftData.event?.location || activeShiftData.location || '',
        role: activeShiftData.role || '',
        hourlyRate: activeShiftData.hourlyRate || 0,
      };

      setActiveShift(mapped);
      setIsAnyShiftActive(true);

      // Calculate work time and break time from break records
      const breaks: any[] = activeShiftData.breaks || [];
      const clockInTime = new Date(activeShiftData.clockIn).getTime();
      let totalBreakMs = 0;
      ...
      breaks.forEach((b: any) => {
        if (b.endTime) {
          ...
          accumulatedBreakMinutes += b.durationMinutes || (breakDuration / 60000);
        }
      });

      if (activeShiftData.status === 'BREAK') {
        // Currently on break - find the active (unclosed) break
        const activeBreak = breaks.find((b: any) => !b.endTime);
        if (activeBreak) {
          // Frozen work time = time from clockIn to when break started, minus previous break time
          const breakStartTime = new Date(activeBreak.startTime).getTime();
          const workBeforeBreak = breakStartTime - clockInTime - totalBreakMs;
          setFrozenWorkTime(Math.max(0, workBeforeBreak));
          setTimerStartTime(new Date(activeBreak.startTime));
          setCurrentShiftStatus('break');
        }
      } else {
        // IN_PROGRESS - calculate frozen work time (total work minus breaks)
        // If there were previous breaks, frozenWorkTime is the work done before those breaks
        // Timer should continue from the last break end (or clockIn if no breaks)
        const lastCompletedBreak = breaks.filter((b: any) => b.endTime).pop();
        if (lastCompletedBreak) {
          // Work done before breaks
          setFrozenWorkTime(now - clockInTime - totalBreakMs - (now - new Date(lastCompletedBreak.endTime).getTime()));
          setTimerStartTime(new Date(lastCompletedBreak.endTime));
        } else {
          // No breaks taken yet
          setFrozenWorkTime(0);
          setTimerStartTime(new Date(activeShiftData.clockIn));
        }
        setCurrentShiftStatus('in-progress');
      }

      setTotalBreakMinutes(accumulatedBreakMinutes);
    }).catch(() => {
  ...
  const startBreak = async () => {
    ...
    try {
      await api.post(`/shifts/${activeShift.id}/break-in`);
      if (timerStartTime) {
        const duration = getSegmentDuration();
      ...
      toast.error(e?.response?.data?.error || "Failed to start break");
    }
  };
  ...
  const endBreak = async () => {
    if (!activeShift) return;
    try {
      const res = await api.post(`/shifts/${activeShift.id}/break-out`);
      if (res.data?.durationMinutes) {
        setTotalBreakMinutes(prev => prev + res.data.durationMinutes);
      }
      ...
      toast.error(e?.response?.data?.error || "Failed to end break");
    }
  };
