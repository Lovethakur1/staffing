...
export function ClientDetail({ userRole, userId, clientId = "client-1" }: ClientDetailProps) {
  ...
  useEffect(() => {
    const fetchClientData = async () => {
      try {
        setIsLoading(true);
        // Default to a known ID for testing if none is passed in URL bounds
        const idToFetch = clientId === "client-1" ? "some-live-id" : clientId;
        const res = await clientService.getClient(idToFetch);
    ...
    // In a real scenario we only fetch if we have a valid UUID. For demo, we might mock if it fails.
    fetchClientData();
  }, [clientId]);

  // Use the fetched profile if available, else fallback to mock structure
  const client = clientProfile ? {
    id: clientProfile.id,
    name: clientProfile.user?.name || "Unknown",
    company: clientProfile.company || "Unknown",
    email: clientProfile.user?.email || "",
    phone: clientProfile.user?.phone || "",
    location: clientProfile.address || "",
    joinDate: clientProfile.user?.createdAt || "2023-01-15",
    status: clientProfile.type || "active",
    totalEvents: clientProfile.totalEvents || 0,
    totalSpent: clientProfile.totalSpent || 0,
    lifetimeValue: clientProfile.totalSpent || 0,
    averageRating: clientProfile.rating || 0,
    outstandingBalance: 0,
    preferredEventTypes: ["Corporate", "Networking"],
    accountManager: "Unassigned",
    notes: "Fetched from live API"
  } : {
    id: clientId,
    name: "Loading...",
    company: "Loading...",
  ...
  // Maps live events if passed in the relation, else empty array
  const liveEvents = clientProfile?.events || [];
  const liveInvoices = clientProfile?.invoices || [];

  // Booking History - Mix live mapped events or fallback to empty
  const bookings = liveEvents.length > 0 ? liveEvents.map((evt: any) => ({
    id: evt.id,
    eventName: evt.title,
    date: evt.date,
    venue: evt.venue || evt.location,
    staffCount: evt.staffRequired || 0,
    totalCost: evt.budget || 0,
    status: evt.status.toLowerCase(),
    rating: undefined,
    feedback: undefined
  })) : [];

  // Invoice History - Map live invoices or fallback
  const invoices = liveInvoices.length > 0 ? liveInvoices.map((inv: any) => ({
    id: inv.invoiceNumber || inv.id,
    eventName: "Event Invoice",
    amount: inv.amount,
