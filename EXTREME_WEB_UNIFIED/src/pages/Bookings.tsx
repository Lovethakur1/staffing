...
export function Bookings({ userRole, userId }: BookingsProps) {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const d = res.data;
        setAllEvents(Array.isArray(d) ? d : (d?.data || d?.events || []));
      } catch (err: any) {
        setFetchError(err?.response?.data?.error || "Failed to load bookings.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // For clients, the backend already filters by clientId via JWT
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">
                          {booking?.shifts?.length || 0}/{booking.staffRequired}
                        </span>
                      </div>
                    </TableCell>
                    <TableCell>
