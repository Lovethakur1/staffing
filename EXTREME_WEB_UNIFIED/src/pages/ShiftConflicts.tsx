    ...
    const detectConflicts = async () => {
      try {
        ...
        for (const shift of shifts) {
          const sid = shift.staffId || 'unknown';
          if (!byStaff[sid]) byStaff[sid] = [];
          byStaff[sid].push(shift);
        }
        ...
        for (const [staffId, staffShifts] of Object.entries(byStaff)) {
          for (let i = 0; i < staffShifts.length; i++) {
            for (let j = i + 1; j < staffShifts.length; j++) {
              const a = staffShifts[i];
              const b = staffShifts[j];
              if (a.date === b.date) {
                const aStart = a.startTime?.replace(':', '') || '0';
                const aEnd = a.endTime?.replace(':', '') || '0';
                const bStart = b.startTime?.replace(':', '') || '0';
                const bEnd = b.endTime?.replace(':', '') || '0';
                const overlap = aStart < bEnd && bStart < aEnd;
                if (overlap) {
                  detected.push({
                    id: `conflict-${a.id}-${b.id}`,
                    staffId,
                    staffName: a.staff?.user?.name || 'Staff Member',
                    type: 'double-booking',
                    severity: 'critical',
                    events: [
                      {
                        id: a.eventId,
                        name: a.event?.title || a.event?.eventName || 'Event A',
                        date: a.date,
                        startTime: a.startTime,
                        endTime: a.endTime,
                        venue: a.event?.location || a.event?.venue || '',
                        client: a.event?.client?.user?.name || ''
                      },
                      {
                        id: b.eventId,
                        name: b.event?.title || b.event?.eventName || 'Event B',
                        date: b.date,
                        startTime: b.startTime,
                        endTime: b.endTime,
                        venue: b.event?.location || b.event?.venue || '',
                        client: b.event?.client?.user?.name || ''
                      }
                    ],
                    description: 'Staff member assigned to two overlapping shifts on the same day',
        ...
        <CardContent>
          <div className="space-y-4">
            {conflicts.filter(c => c.status !== 'resolved').map((conflict) => (
              <div
                key={conflict.id}
                className={`p-4 border-l-4 rounded-lg ${conflict.severity === 'critical' ? 'border-l-red-500 bg-red-50' :
