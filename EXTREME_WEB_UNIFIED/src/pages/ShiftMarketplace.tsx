...
export function ShiftMarketplace({ userRole, userId }: ShiftMarketplaceProps) {
  ...
  const [shiftListings, setShiftListings] = useState<ShiftListing[]>([]);
  ...
  useEffect(() => {
    const fetchShifts = async () => {
      try {
        ...
        // Map raw API shifts to ShiftListing format used by UI. 
        // Real logic will need to handle actual open/pending requests on shifts
        const mapped: ShiftListing[] = shifts.map((s: any) => ({
          id: s.id,
          originalStaffId: s.staffId || 'Unknown',
          originalStaffName: s.staff?.user?.name || 'Open Shift',
          originalStaffRating: s.staff?.staffProfile?.rating || 4.5,
          eventId: s.eventId || '',
          eventName: s.event?.title || s.event?.eventName || 'Event',
          date: s.date || new Date().toISOString(),
          startTime: s.startTime || '09:00',
          endTime: s.endTime || '17:00',
          role: s.role || 'Staff',
          location: s.event?.location || s.event?.venue || 'TBA',
          pay: s.hourlyRate ? (s.hourlyRate * (s.guaranteedHours || 4)) : 150,
          isUrgent: s.status === 'PENDING',
          postedDate: new Date().toISOString(), // Mocked posted date for now
          reason: "Staffing needed",
          requirements: [],
          status: s.status === 'CONFIRMED' ? 'filled' : 'available',
        }));
        setShiftListings(mapped);
      } catch (err) {
        toast.error("Failed to load shift marketplace listings");
      } finally {
        setLoading(false);
      }
    };
