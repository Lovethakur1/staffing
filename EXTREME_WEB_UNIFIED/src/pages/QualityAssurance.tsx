  ...
  useEffect(() => {
    const fetchFeedback = async () => {
      try {
        ...
        setAllFeedback(data.map((f: any) => ({
          id: f.id,
          eventId: f.eventId || '',
          eventName: f.event?.title || f.eventName || '',
          clientId: f.clientId || '',
          clientName: f.client?.user?.name || f.clientName || '',
          eventDate: f.eventDate || f.event?.date || '',
          feedbackDate: f.feedbackDate || f.createdAt || '',
          overallRating: f.overallRating || 0,
          status: (f.status || 'pending').toLowerCase() as EventFeedback['status'],
          responses: f.responses || { staffQuality: 0, professionalism: 0, punctuality: 0, communication: 0, valueForMoney: 0 },
          staffRatings: f.staffRatings || [],
          comments: f.comments || '',
          issues: f.issues || [],
          wouldRecommend: f.wouldRecommend ?? false,
          sentiment: (f.sentiment || 'neutral') as EventFeedback['sentiment'],
        })));
        ...
        <Dialog open={showDetailDialog} onOpenChange={setShowDetailDialog}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Complete feedback and ratings from the client
              </DialogDescription>
            </DialogHeader>
