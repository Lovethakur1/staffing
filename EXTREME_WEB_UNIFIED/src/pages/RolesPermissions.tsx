...
export function RolesPermissions({ userRole, userId }: PageProps) {
    ...
    const fetchUsers = async () => {
      try {
        ...
        const mapped: SystemUser[] = raw.map((u: any) => ({
          ...
          email: u.email || '',
          role: ((u.role || 'staff') as string).toLowerCase().replace('_', '') as UserRole,
          department: u.department || 'General',
          joinDate: u.createdAt?.split('T')[0] || '',
          lastActive: u.updatedAt ? new Date(u.updatedAt).toLocaleString() : 'N/A',
          status: (u.status || 'active') === 'ACTIVE' || u.status === 'active' ? 'active' : 'inactive',
        }));
        setUsers(mapped);
      } catch {
  ...
  const handleSaveRole = async () => {
    if (selectedUser && editingRole) {
      // Map local role name to backend enum format
      const roleMap: Record<string, string> = {
        admin: 'ADMIN', subadmin: 'SUB_ADMIN', manager: 'MANAGER',
        scheduler: 'SCHEDULER', staff: 'STAFF'
      };
      try {
        await api.put(`/users/${selectedUser.id}`, { role: roleMap[editingRole] || editingRole.toUpperCase() });
