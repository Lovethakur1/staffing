    ...
    const fetchData = async () => {
      try {
        ...
        const [evRes, shiftRes] = await Promise.all([
          currentEventId ? api.get(`/events/${currentEventId}`) : Promise.resolve({ data: null }),
          api.get(`/shifts${currentEventId ? `?eventId=${currentEventId}` : ''}`),
        ]);
        ...
        const records: AttendanceRecord[] = shiftArr.map((s: any) => {
          let status: AttendanceRecord['status'] = 'absent';
          if (s.status === 'COMPLETED') {
            status = 'checked-out';
          } else if (s.status === 'BREAK') {
            // Staff is currently on break
            status = 'on-break';
            ...
            if (s.clockIn && s.startTime) {
              ...
              status = clockInTime > scheduled ? 'late' : 'checked-in';
            } else {
              status = 'checked-in';
            }
          } else if (s.status === 'CANCELLED' || s.status === 'NO_SHOW' || s.status === 'PENDING') {
            status = 'absent';
    ...
    const intervalId = setInterval(() => {
      ...
      const silentFetch = async () => {
        try {
          ...
          // Silent fail - don't update error state on polling
        }
      };
      silentFetch();
    }, 10000);
