...
export function EventStaffDetails({ userRole, userId }: EventStaffDetailsProps) {
  ...
  const [favoriteStaff, setFavoriteStaff] = useState<string[]>([
    "staff-1", "staff-3", "staff-5" // Mock favorites
  ]);

  // Exclusion State
  const [excludeDialogOpen, setExcludeDialogOpen] = useState(false);
  const [selectedStaffForExclusion, setSelectedStaffForExclusion] = useState<Staff | null>(null);
  const [exclusionReason, setExclusionReason] = useState("");

  const handleExcludeClick = (staff: Staff) => {
  ...
  const eventManager = mockUsers.find(user => user.id === event.managerId);
  const eventStaff = mockStaff.filter(staff => event.assignedStaff.includes(staff.id));
                            ...
                            <div className="flex gap-1">
                              {[1, 2, 3, 4, 5].map((star) => (
                                ...
                                >
                                  <Star 
                                    className={`w-5 h-5 ${
                                      star <= (groupRatings[staff.id]?.rating || 5)
                                        ? 'fill-warning text-warning' 
                                        : 'text-muted-foreground'
                                    }`} 
                                  />
