...
import { Calendar, Clock, MapPin, Users, DollarSign, Star, Plus, Minus, Check, ChevronLeft, ChevronRight, Heart, AlertCircle } from "lucide-react";
...
export function EnhancedBookingForm({ isOpen, onClose, clientId }: EnhancedBookingFormProps) {
  ...
  const handleSubmit = () => {
    // Validation
    if (!eventData.title || !eventData.eventType || !startDate || !endDate || !eventData.startTime || !eventData.endTime) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="mobile-dialog-content max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-center space-x-2 sm:space-x-4 mb-4 sm:mb-6 px-4">
          {[1, 2, 3].map((stepNumber) => (
            <div key={stepNumber} className="flex items-center">
              <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-sm sm:text-base transition-all ${
                stepNumber === step ? 'bg-primary text-primary-foreground ring-4 ring-primary/20' :
                stepNumber < step ? 'bg-success text-success-foreground' :
                'bg-muted text-muted-foreground'
              }`}>
                {stepNumber < step ? <CheckCircle2 className="h-4 w-4 sm:h-5 sm:w-5" /> : stepNumber}
              </div>
              {stepNumber < 3 && (
                <div className={`w-8 sm:w-12 md:w-16 h-1 mx-1 sm:mx-2 transition-all ${
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select value={eventData.startTime} onValueChange={(value) => setEventData({...eventData, startTime: value})}>
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select value={eventData.endTime} onValueChange={(value) => setEventData({...eventData, endTime: value})}>
                      <SelectTrigger>
