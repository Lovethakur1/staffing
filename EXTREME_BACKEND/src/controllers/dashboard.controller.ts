...
export const getDashboardStats = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const recentCompletedEvents = await prisma.event.count({
    where: {
      date: { gte: weekStart },
      status: 'COMPLETED',
    },
  ...
  const onTimeCount = weekTimesheets.filter(t => {
    if (!t.clockInTime || !t.shift?.startTime) return true;
 ...
 */
export const getLiveMetrics = asyncHandler(async (req: AuthRequest, res: Response) => {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const tomorrow = new Date(today);
  tomorrow.setDate(tomorrow.getDate() + 1);
  ...
  const yesterdayEvents = await prisma.event.count({
    where: {
      date: { gte: yesterday, lt: today },
  ...
  const pendingCheckOuts = shiftsToday.filter(s => 
    (s.status === 'ONGOING' || s.status === 'IN_PROGRESS') && !s.timesheet?.clockOutTime
  ).length;

  // Late staff (clocked in more than 10 mins after start time)
  const lateStaff = shiftsToday.filter(s => {
    if (!s.timesheet?.clockInTime || !s.startTime) return false;
  ...
  // Unique active staff
  const activeStaffIds = new Set(shiftsToday.filter(s => 
    ['CONFIRMED', 'TRAVEL_TO_VENUE', 'ARRIVED', 'ONGOING', 'IN_PROGRESS'].includes(s.status)
  ).map(s => s.staffId));

  // Shift rejections (REJECTED status)
  const shiftRejections = await prisma.shift.count({
    where: {
      date: { gte: today },
      status: 'REJECTED',
    },
 ...
 */
export const getUpcomingCriticalEvents = asyncHandler(async (req: AuthRequest, res: Response) => {
  const today = new Date();
 ...
 */
export const getPendingActions = asyncHandler(async (req: AuthRequest, res: Response) => {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
 ...
 */
export const getLiveOperations = asyncHandler(async (req: AuthRequest, res: Response) => {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const tomorrow = new Date(today);
  tomorrow.setDate(tomorrow.getDate() + 1);
 ...
 */
export const getEventRequests = asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  const requests = events.map((event, idx) => {
    ...
    if (event.status === 'CONFIRMED') displayStatus = 'approved';
    else if (event.status === 'CANCELLED') displayStatus = 'rejected';
    else if (event.status === 'IN_PROGRESS') displayStatus = 'under-review';
    ...
    let priority = 'medium';
    if (daysUntilEvent <= 2) priority = 'urgent';
    else if (daysUntilEvent <= 7) priority = 'high';
    else if (daysUntilEvent > 30) priority = 'low';
...
function formatRelativeDate(date: Date): string {
  ...
  if (diffDays < 7) return targetDate.toLocaleDateString('en-US', { weekday: 'long' });
