  ...
  const handleClockAction = () => {
    if (currentUser.role === 'staff') {
      if (!contextActiveShift) {
        ...
        // Only allow clock out if status is appropriate
        if (currentShiftStatus === 'in-progress' || currentShiftStatus === 'break') {
          ...
          console.log('Clocked out from shift');
           ...
           // For traveling/arrived states, we shouldn't clock out here usually
           // But if the user forces it via this dialog, we might want to allow it or just close
           // For now, let's treat it as "View Details" essentially if it's not a clock-out-able state
        }
      }
