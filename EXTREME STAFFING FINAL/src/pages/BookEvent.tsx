...
import { mockClients, mockStaff, FavoriteEvent, Client } from "../data/mockData";
...
export function BookEvent({
  ...
  // Get client data for favorite events
  const currentClient = mockClients.find(client => client.id === userId) as Client;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulate sending request to admin panel
    toast.success(
      "Event booking request sent to admin for review! You'll receive confirmation within 24 hours.",
    );
            ...
            <SelectContent>
              ...
              {favoriteEvents.map((favoriteEvent) => {
                const staffNames = favoriteEvent.staffIds
                  .map(staffId => mockStaff.find(s => s.id === staffId)?.name)
                  .filter(Boolean)
