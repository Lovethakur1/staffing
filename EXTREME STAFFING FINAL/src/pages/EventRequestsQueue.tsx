...
import { eventRequestsData, EventRequest } from "../data/eventRequestsData";
...
export function EventRequestsQueue({ userRole, userId }: EventRequestsQueueProps) {
  ...
  const handleViewDetails = (request: EventRequest) => {
    setPageParams({ requestId: request.id });
    setCurrentPage("event-request-detail");
  };

  const handleApproveAndCreate = (request: EventRequest) => {
    toast.success(`Request approved! Creating event: ${request.eventName}`);
    setTimeout(() => {
