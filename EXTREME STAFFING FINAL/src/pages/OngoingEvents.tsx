...
export function OngoingEvents({ userRole, userId }: OngoingEventsProps) {
  ...
  // 1. Filter by Role & Date (Ongoing/Today)
  const baseEvents = useMemo(() => {
    let events = isClient 
      ? mockEvents.filter(event => event.clientId === userId)
      : mockEvents;
    ...
    return events.filter(event => {
      // Logic: Event is ongoing if status is 'in-progress' OR (status is 'confirmed' AND date is today)
      // Note: In a real app, we'd also check start/end time vs current time
      const eventDate = event.date; // event.date is YYYY-MM-DD string
      return event.status === 'in-progress' || (event.status === 'confirmed' && eventDate === todayStr);
    });
  }, [isClient, userId]);

  // 2. Get Unique Event Types for Filter
  const eventTypes = useMemo(() => {
    const types = new Set(baseEvents.map(e => e.eventType).filter(Boolean));
  ...
  const activeStaffCount = baseEvents.reduce((acc, event) => acc + event.assignedStaff.length, 0);
              ...
              <TableBody>
                {paginatedEvents.length > 0 ? (
                  paginatedEvents.map((event, index) => {
                    ...
                    const staffingPercentage = event.staffRequired > 0 
                      ? Math.round((event.assignedStaff.length / event.staffRequired) * 100) 
                      : 0;
                    
                    const assignedStaffDetails = event.assignedStaff
                        ...
                        <TableCell>
                          <div className="w-[160px] space-y-2">
                            <div className="flex justify-between text-xs">
                              <span className="text-muted-foreground">{event.assignedStaff.length}/{event.staffRequired} Staff</span>
                            ...
                            <div className="flex -space-x-2">
                              {assignedStaffDetails.slice(0, 3).map((staff, i) => (
                                <Avatar key={staff?.id || i} className="h-6 w-6 border-2 border-background ring-1 ring-border/50">
                        ...
                        <TableCell className="text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem onClick={() => setCurrentPage("messages", { eventId: event.id, staffIds: event.assignedStaff, eventTitle: event.title })}>
